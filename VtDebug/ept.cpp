...
static UCHAR g_eptp_mtrr_default_type;
 ...
 void EptpDestructTables(_In_ EptCommonEntry* table,
...
_Use_decl_annotations_ static void EptpInitTableEntry(
    ...
    entry->fields.physial_address = Uti::UtilPfnFromPa(physical_address);
    if (table_level == 1) {
...
_Use_decl_annotations_ static EptCommonEntry* EptpConstructTables(
    ...
    case 4: {
        ...
        if (!ept_pml4_entry->all) {
            ...
            EptpInitTableEntry(ept_pml4_entry, table_level, Uti::UtilPaFromVa(ept_pdpt));
        }
        return EptpConstructTables(
            reinterpret_cast<EptCommonEntry*>(
                Uti::UtilVaFromPfn(ept_pml4_entry->fields.physial_address)),
    ...
    case 3: {
        ...
        if (!ept_pdpt_entry->all) {
            ...
            EptpInitTableEntry(ept_pdpt_entry, table_level, Uti::UtilPaFromVa(ept_pdt));
        }
        return EptpConstructTables(
            reinterpret_cast<EptCommonEntry*>(
                Uti::UtilVaFromPfn(ept_pdpt_entry->fields.physial_address)),
            table_level - 1, physical_address, ept_data);
    }
    case 2: {
        // table == PDT (2 MB)
        ...
        if (!ept_pdt_entry->all) {
            ...
            EptpInitTableEntry(ept_pdt_entry, table_level, Uti::UtilPaFromVa(ept_pt));
        }
        return EptpConstructTables(
            reinterpret_cast<EptCommonEntry*>(
                Uti::UtilVaFromPfn(ept_pdt_entry->fields.physial_address)),
            table_level - 1, physical_address, ept_data);
    }
    case 1: {
        // table == PT (4 KB)
...
_Use_decl_annotations_ memory_type EptpGetMemoryType(ULONG64 physical_address) {
...
_Use_decl_annotations_  ULONG64 EptpAddressToPxeIndex(
...
_Use_decl_annotations_  ULONG64 EptpAddressToPpeIndex(
    ULONG64 physical_address) {
...
_Use_decl_annotations_  ULONG64 EptpAddressToPdeIndex(
    ULONG64 physical_address) {
...
_Use_decl_annotations_  ULONG64 EptpAddressToPteIndex(
    ULONG64 physical_address) {
    const auto index = (physical_address >> kEptpPtiShift) & kEptpPtxMask;
    return index;
}
_Use_decl_annotations_  void EptpFreeUnusedPreAllocatedEntries(EptCommonEntry** preallocated_entries, long used_count)
{
...
_Use_decl_annotations_  void EptpDestructTables(EptCommonEntry* table,ULONG table_level)
{
    for (auto i = 0ul; i < 512; ++i) {
        const auto entry = table[i];
        if (entry.fields.physial_address) {
            const auto sub_table = reinterpret_cast<EptCommonEntry*>(
                Uti::UtilVaFromPfn(entry.fields.physial_address));
...
namespace ept
{
    ...
    EptCommonEntry* EptpGetEptPtEntry(_In_ EptCommonEntry* table,
        _In_ ULONG table_level,
        _In_ ULONG64 physical_address);
    ...
    _Use_decl_annotations_ bool EptIsEptAvailable() {
      ...
      */
      ...
      Ia32VmxEptVpidCapMsr capability = { Uti::UtilReadMsr64(Msr::kIa32VmxEptVpidCap)};
    ...
    void EptInitializeMtrrEntries()
    {
        ...
        Ia32MtrrDefaultTypeMsr default_type = { Uti::UtilReadMsr64(Msr::kIa32MtrrDefType) };
        ...
        Ia32MtrrCapabilitiesMsr mtrr_capabilities = {
            Uti::UtilReadMsr64(Msr::kIa32MtrrCap) };
        Log("MTRR Default=%llu, VariableCount=%llu, FixedSupported=%llu, "
            "FixedEnabled=%llu \n",
        ...
        if (mtrr_capabilities.fields.fixed_range_supported &&
            ...
            Ia32MtrrFixedRangeMsr fixed_range = {
                Uti::UtilReadMsr64(Msr::kIa32MtrrFix64k00000) };
            ...
            for (auto msr = static_cast<ULONG>(Msr::kIa32MtrrFix16k80000);
                ...
                fixed_range.all = Uti::UtilReadMsr64(static_cast<Msr>(msr));
                for (auto memory_type : fixed_range.fields.types) {
        ...
        for (auto i = 0; i < mtrr_capabilities.fields.variable_range_count; i++) {
            ...
            Ia32MtrrPhysMaskMsr mtrr_mask = { Uti::UtilReadMsr64(static_cast<Msr>(phy_mask)) };
            ...
            Ia32MtrrPhysBaseMsr mtrr_base = { Uti::UtilReadMsr64(static_cast<Msr>(phy_base)) };
    ...
    // Returns an EPT entry corresponds to the physical_address
    _Use_decl_annotations_  EptCommonEntry* EptpGetEptPtEntry(
        EptCommonEntry* table, ULONG table_level, ULONG64 physical_address) 
    {
        if (!table) {
            return nullptr;
        ...
        case 4: {
            ...
            return EptpGetEptPtEntry(reinterpret_cast<EptCommonEntry*>(Uti::UtilVaFromPfn(
    ...
    _Use_decl_annotations_ EptData* EptInitialization() {
        ...
        ept_poiner->fields.memory_type = static_cast<ULONG64>(EptpGetMemoryType(Uti::UtilPaFromVa(ept_pml4)));
        ept_poiner->fields.page_walk_length = kEptPageWalkLevel - 1;
        ept_poiner->fields.pml4_address = Uti::UtilPfnFromPa(Uti::UtilPaFromVa(ept_pml4));

        // Initialize all EPT entries for all physical memory pages
        const auto pm_ranges = Uti::UtilGetPhysicalMemoryRanges();
        ...
        const Ia32ApicBaseMsr apic_msr = { Uti::UtilReadMsr64(Msr::kIa32ApicBase) };
    ...
    // Deal with EPT violation VM-exit. 26.5.7.1 Convertible EPT Violations
    _Use_decl_annotations_ void EptHandleEptViolation(EptData* ept_data) {
        const EptViolationQualification exit_qualification = { Uti::UtilVmRead(VmcsField::kExitQualification) };

        const auto fault_pa = Uti::UtilVmRead64(VmcsField::kGuestPhysicalAddress);
        const auto fault_va = reinterpret_cast<void*>(
            exit_qualification.fields.valid_guest_linear_address
            ? Uti::UtilVmRead(VmcsField::kGuestLinearAddress)
            : 0);
        ...
        */
        // EPT entry miss. It should be device memory. 重新挂一个物理页(看可以把他看成 操作系统的缺页异常处理)
        NT_ASSERT(EptpIsDeviceMemory(fault_pa));
        EptpConstructTables(ept_data->ept_pml4, 4, fault_pa, ept_data);

        Uti::UtilInveptGlobal(); //刷一刷 tlb
    }
